      ...
      <div ref={ref} className="max-w-6xl mx-auto">
        ...
        >
          <div className="flex items-baseline gap-3 mb-3 flex-wrap">
            ...
            <span className="hand text-gold text-2xl rotate-n2 inline-block">artist drops every 6–8 weeks</span>
          </div>
          ...
          <p className="font-serif text-xl text-cream-300 mt-4 max-w-2xl">
            Community artist collabs. Each Limited NFT adds 15–40 Cocoa Units
            based on rarity — stacks on top of your OGs.{' '}
            ...
            {keys.map(k => (
              <div key={k} className="pb-4 border-b border-cream-500/10">
                ...
                <div className="flex flex-wrap gap-2">
                  {TRAITS[k].options.map((opt, i) => {
                    const active = sel[k] === i
                    return (
                      <button
                        key={opt}
                        onClick={() => { setSel(p => ({ ...p, [k]: i })); setMinted(false) }}
                        className={`px-4 py-1.5 rounded-full font-serif text-base transition-all ${
                          active
                            ? 'bg-cream-50 text-cocoa-900 font-semibold'
                            : 'text-cream-400 hover:text-cream-100 italic'
                        }`}
                      >
                        {opt}
                      </button>
              ...
              <div className="bg-cream-50 p-3 rotate-p2 shadow-2xl">
                ...
                <div className="py-3 px-2">
                  ...
                  <p className="mono text-xs text-cocoa-700 mt-1">
                    {keys.map(k => TRAITS[k].options[sel[k]]).join(' · ')}
                  </p>
                </div>
              </div>
              {connected && (
